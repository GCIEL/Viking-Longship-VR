...
namespace Valve.VR.InteractionSystem
{
	...
	public class Teleport : MonoBehaviour
    {
		...
		private void TeleportPlayer()
		{
			...
			{
				Vector3 playerFeetOffset = player.trackingOriginTransform.position - player.feetPositionGuess;
				if(player.GetComponent<CharacterController>() != null) player.GetComponent<CharacterController>().enabled = false;
				player.trackingOriginTransform.position = teleportPosition + playerFeetOffset;
				if(player.GetComponent<CharacterController>() != null) player.GetComponent<CharacterController>().enabled = true;
